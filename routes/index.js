...
router.post('/declarations', (req, res) => {
    ...
    const { g, empID, c, d, e, ccd,ggg, ccc, dd, age,gross_total} = dec;
    // var agebased;
    var gg = parseInt(g);
    ...
    mysqldb.query(`select * from Employees`, (err, result) => {
        if (err) {
            ...
            mysqldb.query(`select * from edit_limits`, (err, upper) => {
                ...
                var gglimit=JSON.parse(JSON.stringify(upper))[0].gglimit

                //not adding 80gg rent
...
router.get('/masterview', ensureAuthenticated, (req, res) => {
    ...
    var cur_month = mlist[new Date().getMonth()]
    var cur_year = new Date().getFullYear()
    mysqldb.query(`select * from Salary right join Employees where month="${cur_month}" and year="${cur_year}"`, (err, result) => {
        if (err) {
            console.log(err);
        }
        else {
...
router.get('/master-view-prev-month', ensureAuthenticated, (req, res) => {
    ...
    var cur_year = new Date().getFullYear()
    mysqldb.query(`select * from Salary right join Employees where month="${prev_month}" and year="${cur_year}"`, (err, result) => {
        if (err) {
            console.log(err);
        }
        else {
                                    ...
                                    if (err) {
                                        ...
                                        mysqldb.query(`select * from advance_temp where empID='${empID}'`,(err,result)=>{
                                            if (err) {
                                                ...
                                                mysqldb.query(`select groupInsurance from group_insurance right join Employees where empID='${empID}' and month='${month}' and year=${year}`,(err,result)=>{
                                                    if(err)
                                                    {
                                                        console.log(err)
...
router.get('/uploads/:empID', (req, res) => {
    ...
    if (requestedTitle.includes("EMP")) {
        mysqldb.query(`select * from Salary right join Employees`, (err, result) => {
            if (err) {
                //console.log(err);
            }
            else {
                    ...
                    for (let i = 0; i < length; i++) {
                        ...
                        mysqldb.query(`select E.empID,E.age,IFNULL(f.total,0) as total,E.emp_temp_regime,IFNULL(f.gross_sal,50000) as gross_sal from Employees E left join form f on(E.empID=f.empID) ORDER BY E.empID LIMIT ${i},1`, (err, result) => {
                            if (err) {
                                ...
                                var net_taxable_income = gross_total_income - exemption-st_ded;
