...
try {
  ...
  router.get("/newdeclaration", ensureAuthenticated, (req, res) => {
    mysqldb.query(
      `select * from Employees natural join form`,
      (err, result) => {
        if (err) {
          console.log(err);
        } else {
  ...
  router.get("/salcert/:empID", ensureAuthenticated, (req, res) => {
    ...
    mysqldb.query(
      `   SELECT * FROM Salary natural join employees where empID="${requestedTitle}"    ORDER BY STR_TO_DATE(CONCAT(year, month, ' 01'), '%Y %M %d');  
    `,
      (err, result) => {
        if (err) {
          console.log(err);
  ...
  router.get("/salcert1/:empID", ensureAuthenticated, (req, res) => {
    ...
    // mysqldb.query(`select * from Salary natural join Employees where (month='${cur_month}' or month='${cur_month_2}' or month='${cur_month_3}'or month='${cur_month_4}'or month='${cur_month_5}'or month='${cur_month_1}') and year=${cur_year} and empID='${requestedTitle}' `, (err, result) => {
    mysqldb.query(
      `   SELECT * FROM Salary right join Employees ON Salary.empID= Employees.empID where Salary.empID="${req.params.empID}" ORDER BY STR_TO_DATE(CONCAT(year, month, ' 01'), '%Y %M %d');
  ...
  router.get("/salcert2/:empID", ensureAuthenticated, (req, res) => {
    ...
    var cur_year = new Date().getFullYear();
    mysqldb.query(
      `select * from Salary natural join Employees where month='${cur_month}' and year=${cur_year}`,
      (err, result) => {
        if (err) {
          console.log(err);
        } else {
  ...
  router.get("/salsheet", (req, res) => {
    ...
    ON  Salary.empID=Employees.empID where month='${cur_month}' and year= ${cur_year} `,
      (err, result) => {
        if (err) {
          console.log(err);
        } else {
  ...
  router.post("/editcess", (req, res) => {
    ...
    mysqldb.query(
      `update config set educess=${educess} where ID=1`,
      (err, result) => {
        if (err) {
          console.log(err);
        } else {
        ...
        if (err) {
          ...
          mysqldb.query(`select * from edit_limits`, (err, upper) => {
            ...
            var total =
              ...
              Math.min(ded_c, climit) +
              ded_e +
              Math.min(ded_ccc, ccclimit) +
              Math.min(ded_ccd, ccdlimit) +
              Math.min(ded_dd, ddlimit);
            console.log(total);
    ...
    mysqldb.query(
      ...
      (err, result2) => {
        if (err) {
          ...
          mysqldb.query(
            `select count(*) as formCount from form`,
            (err, result) => {
              if (err) {
                console.log(err);
              } else {
                var render = true;
                if (
                  JSON.parse(JSON.stringify(result2))[0].empCount ===
                  JSON.parse(JSON.stringify(result))[0].formCount
                ) {
                  render = false;
                }
                console.log(
  ...
  router.get("/incometax", ensureAuthenticated, (req, res) => {
    mysqldb.query(
      `select * from income_tax natural join form`,
      (err, result) => {
        if (err) {
          console.log(err);
        } else {
  ...
  router.get("/updateit/:empID", ensureAuthenticated, (req, res) => {
    ...
    if (requestedTitle.includes("EMP")) {
      mysqldb.query(
        `select * from form natural join Employees `,
        (err, result) => {
          if (err) {
            console.log(err);
          } else {
            mysqldb.query(
              `select * from Employees natural join form where empID="${req.params.empID}"`,
              (err, result2) => {
                if (err) {
                  console.log(err);
                } else {
  ...
  router.post("/update80c", (req, res) => {
    ...
    mysqldb.query(
      `update eighty_c set epf=${epf}, ppf=${ppf}, nsc=${nsc}, ulip=${ulip}, insurancePremium=${insurancePremium}, houseLoan=${houseLoan}, tuitionFee=${tuitionFee}, bankDeposits=${bankDeposits}, regFee=${regFee} where empID="${empID}"`,
      (err, result) => {
        if (err) {
          console.log(err);
        } else {
  ...
  router.post("/donations", ensureAuthenticated, (req, res) => {
    ...
    if (list2.length == 0) {
      ...
      for (var i in list2) {
        ...
        console.log(cause);
        console.log(
          `insert into donation(empID,donationDays,month,year,cause) VALUES('${list2[i]}',${donateDays},'${cur_month}',${cur_year},'${cause}')`
        );
        mysqldb.query(
          `insert into donation(empID,donationDays,month,year,cause) VALUES('${list2[i]}',${donateDays},'${cur_month}',${cur_year},'${cause}') on duplicate key update donationDays=${donateDays}, cause='${cause}'`,
          (err, result) => {
            if (err) {
              console.log(err);
              res.json({
                status: "error",
                message: "Please Fill No. of Days and Cause",
              });
            } else {
              console.log("abc");
              if (data["cause"] == "") {
                ...
                res.json({
                  status: "success",
                  message: "Donations Added Successfully!",
                });
              }
            }
          }
  ...
  router.get("/ta", ensureAuthenticated, (req, res) => {
    mysqldb.query(
      `select * from Employees where salaryCategory="Pay Scale"`,
      (err, result) => {
        if (err) {
          console.log(err);
        } else {
  ...
  router.post("/ta", ensureAuthenticated, (req, res) => {
    ...
    if (list2.length == 0) {
      ...
      mysqldb.query(
        `update Employees set ta=${newValue} where empID in ${list}`,
        (err, result) => {
          if (err) {
            console.log(err);
            res.json({
  ...
  router.post("/hra", ensureAuthenticated, (req, res) => {
    ...
    if (list2.length == 0) {
      ...
      console.log("list is", list2);
      console.log(
        `update Employees set hra=${newValue} where empID in ${list}`
      );
      mysqldb.query(
        `update Employees set hra=${newValue} where empID in ${list}`,
        (err, result) => {
  ...
  router.post("/da", ensureAuthenticated, (req, res) => {
    ...
    if (list2.length == 0) {
      ...
      mysqldb.query(
        `update Employees set da=${newValue} where empID in ${list}`,
        (err, result) => {
          if (err) {
            console.log(err);
            res.json({
  ...
  router.post("/cca", ensureAuthenticated, (req, res) => {
    ...
    if (list2.length == 0) {
      ...
      console.log("list is", list2);
      mysqldb.query(
        `update Employees set cca=${newValue} where empID in ${list}`,
        (err, result) => {
          if (err) {
            console.log(err);
            res.json({
  ...
  router.post("/differences", ensureAuthenticated, (req, res) => {
    ...
    first_month = mlist[first_month - 1].toLowerCase();
  ...
  router.get("/otherdifferences", ensureAuthenticated, (req, res) => {
    mysqldb.query(
      `select distinct hra from Employees where salaryCategory="Pay Scale"`,
      (err, result3) => {
        if (err) {
          console.log(err);
        } else {
  ...
  router.get("/dadifference", ensureAuthenticated, (req, res) => {
    mysqldb.query(
      `select distinct da from Employees where salaryCategory="Pay Scale"`,
      (err, result3) => {
        if (err) {
          console.log(err);
        } else {
  ...
  router.post("/dadifference", ensureAuthenticated, (req, res) => {
    ...
    for (let i = 0; i < list2.length; i++) {
      console.log("i is", list2[i]);
      mysqldb.query(
        `insert into da_difference(empID,difference,month,duration,year) VALUES ('${
          list2[i]
        }',${da_difference},'${month.toLowerCase()}',${da_duration},${year})`,
        (err, result) => {
          if (err) {
            console.log(err);
          } else {
  ...
  router.post("/otherdifferences", ensureAuthenticated, (req, res) => {
    ...
    for (let i = 0; i < list2.length; i++) {
      ...
      // })
      mysqldb.query(
        `insert into hra_difference(empID,difference,month,duration,year) VALUES ('${
          list2[i]
        }',${hra_difference},'${month.toLowerCase()}',${hra_duration},${year})`,
        (err, result) => {
          if (err) {
            console.log(err);
          } else {
  ...
  router.post("/pay", ensureAuthenticated, (req, res) => {
    ...
    data.month =
      monthNames[
        (monthNames.indexOf(data.month.toLowerCase()) + 1) % 12
      ].toLowerCase();
    if (data.month === 0) data.year = data.year + 1;
    ...
    mysqldb.query(
      `INSERT INTO lwp (empID, month, year, days, lwp) VALUES ('${
        data.empID
      }', '${data["month"].toLowerCase()}', ${data["year"]}, ${days}, ${
        data["lwp"]
        ...
        mysqldb.query(
          ...
          (err, result2) => {
            if (err) {
              ...
              mysqldb.query(
                `update Employees set workexpYr=(floor(DATEDIFF(now(), doj)/ 365.2425)) where pay>0`,
                (err, result2) => {
                  if (err) {
                    console.log(err);
                  } else {
  ...
  router.get("/finalrecoveryamt", ensureAuthenticated, (req, res) => {
    ...
    mysqldb.query(
      `select * from Employees natural join recovery`,
      (err, result) => {
        if (err) {
          console.log(err);
        } else {
  ...
  router.get("/finaladvances", ensureAuthenticated, (req, res) => {
    mysqldb.query(
      `select * from Employees natural join advance_temp`,
      (err, result) => {
        if (err) {
          console.log(err);
        } else {
  ...
  router.get("/finalmiscellaneous", ensureAuthenticated, (req, res) => {
    ...
    mysqldb.query(
      `select * from Employees natural join miscellaneous`,
      (err, result) => {
        if (err) {
          console.log(err);
        } else {
  ...
  router.get("/finalattendance", ensureAuthenticated, (req, res) => {
    ...
    ON Employees.empID= late_attendance.empID`,
      (err, result) => {
        if (err) {
          console.log(err);
        } else {
  ...
  router.get("/groupinsurance", ensureAuthenticated, (req, res) => {
    ...
    mysqldb.query(
      `select * from Employees where empID not in (select empID from group_insurance where year=${year})`,
      (err, result) => {
        if (err) {
          console.log(err);
        } else {
  ...
  router.post("/groupinsurance", ensureAuthenticated, (req, res) => {
    ...
    if (IDlist.length == 0) {
      ...
      for (let i = 0; i < IDlist.length; i++) {
        console.log(
          `insert into group_insurance (empID,month,year) VALUES ('${IDlist[i]}',${cur_month},${year})`
        );
        mysqldb.query(
          `insert into group_insurance (empID,month,year) VALUES ('${IDlist[i]}','${cur_month}','${year}') `,
          (err, result) => {
            if (err) {
              ...
              res.json({
                status: "error",
                message: "Group Insurance Already Added!",
              });
            } else {
      ...
      res.json({
        status: "success",
        message: "Group Insurance Added Successfully!",
      });
    }
  ...
  router.get("/declarations", ensureAuthenticated, (req, res) => {
    mysqldb.query(
      `select * from Employees where empID NOT IN (select empID from form)`,
      (err, result) => {
        if (err) {
          console.log(err);
        } else {
  ...
  router.post("/advances", ensureAuthenticated, (req, res) => {
    ...
    console.log(
      `INSERT INTO advance_temp (empID, ,amount, month, year, duration, outstanding) VALUES ('${empID}', ${data["amt"]}, '${data["month"]}', ${data["year"]}, ${data["duration"]}, ${data["amt"]})`
    );
    mysqldb.query(
      `INSERT INTO advance_temp (empID, amount, month, year, duration, outstanding) VALUES ('${empID}', ${data["amt"]}, '${data["month"]}', ${data["year"]}, ${data["duration"]}, ${data["amt"]}) ON DUPLICATE KEY UPDATE
    amount = ${data["amt"]},  
    ...
    mysqldb.query(
      ...
      (err, result) => {
        ...
        for (let i = 0; i < result.length; i++) {
          mysqldb.query(
            `update Employees set pay=${result[i].updatedPay} where empID='${result[i].empID}'`,
            (err, result) => {
              if (err) {
                console.log(err);
                res.json({
  ...
  router.get("/masterview", ensureAuthenticated, (req, res) => {
    ...
    var cur_year = new Date().getFullYear();
    mysqldb.query(
      `select * from Salary right join Employees where month="${cur_month}" and year="${cur_year}"`,
      (err, result) => {
        if (err) {
          console.log(err);
        } else {
  ...
  router.get("/master-view-prev-month", ensureAuthenticated, (req, res) => {
    ...
    var cur_year = new Date().getFullYear();
    mysqldb.query(
      `select * from Salary right join Employees where month="${prev_month}" and year="${cur_year}"`,
      (err, result) => {
        if (err) {
          console.log(err);
        } else {
      ...
      (err, result) => {
        ...
        // mysqldb.query(`call while_example();`)
          ...
          mysqldb.query(`select * from trial`, (err, result2) => {
            ...
            var str = JSON.parse(JSON.stringify(result2))[result2.length - 1]
              .abc;
    ...
    function calculateSalary(item, i, callback) {
      ...
      mysqldb.query(`select * from config`, (err, result) => {
        if (err) {
          ...
          var prov_fund_DNA = JSON.parse(JSON.stringify(result))[0]
            .prov_fund_DNA;
          var prov_fund_Percent = JSON.parse(JSON.stringify(result))[0]
            .prov_fund_Percent;
          var prov_fund_Max = JSON.parse(JSON.stringify(result))[0]
            .prov_fund_Max;
          ...
          var rev_stamp_DNA = JSON.parse(JSON.stringify(result))[0]
            .rev_stamp_DNA;
          var rev_stamp_max = JSON.parse(JSON.stringify(result))[0]
            .rev_stamp_max;
          ...
          mysqldb.query(
            ...
            (err, result) => {
              if (err) {
                ...
                var pay = parseInt(JSON.parse(JSON.stringify(result))[0].pay);
                var ta_temp = parseInt(
                  JSON.parse(JSON.stringify(result))[0].ta
                );
                      ...
                      mysqldb.query(
                        ...
                        (err, result) => {
                          if (err) {
                            console.log(err);
                            console.log(
                              "invalid select from d_difference query"
                            );
                          } else {
  ...
  router.get("/uploads/:empID", (req, res) => {
    ...
    if (requestedTitle.includes("EMP")) {
      mysqldb.query(
        `select * from Salary right join Employees`,
        (err, result) => {
          if (err) {
            //console.log(err);
          } else {
  ...
  router.get("/lwp/:empID", (req, res) => {
    ...
    if (requestedTitle.includes("EMP")) {
      ...
      mysqldb.query(
        `select * from Employees where empID="${requestedTitle}"`,
        (err, result) => {
          if (err) {
            console.log(err);
          } else {
  ...
  router.post("/deductions", (req, res) => {
    ...
    mysqldb.query(
      `update config set prov_fund_DNA=${prov_fund_DNA},prov_fund_Percent=${prov_fund_Percent},prov_fund_Max=${prov_fund_Max},prof_tax_Max=${prof_tax_Max},prof_tax_Percent=${prof_tax_Percent},prof_tax_DNA=${prof_tax_DNA},rev_stamp_max=${rev_stamp_max},rev_stamp_DNA=${rev_stamp_DNA} where ID=1`,
      (err, result) => {
        if (err) {
          console.log(err);
          res.json({
          ...
          res.json({
            status: "success",
            message: "Deductions Edited Successfully",
          });
        }
      }
    );
  ...
  router.post("/deleteEmployee", (req, res) => {
    ...
    mysqldb.query(
      `delete from Employees where empID='${req.body.id}'`,
      (err, result) => {
        if (err) {
          //------------ Invalid registration Number ------------//
          // req.flash('error_msg',
  ...
  router.post("/deleteAttendance", (req, res) => {
    ...
    mysqldb.query(
      `delete from ${req.body.table} where empID='${req.body.empID}'`,
      (err, result) => {
        if (err) {
          console.log(err);
        } else {
  ...
  router.post("/lateattendance", ensureAuthenticated, (req, res) => {
    ...
    if (data.month === 11) data.year = data.year + 1;
    data.month = monthNames[monthNames.indexOf(data.month.toLowerCase()) + 1];
    var prev = data.month;
    prev =
      monthNames[
        (monthNames.indexOf(prev.toLowerCase()) - 1 + 12) % 12
      ].toLowerCase();
    // console.log(length)
    // for (let i = 0; i < length; i++) {
    var prevdays;
    ...
    mysqldb.query(
      `INSERT INTO late_attendance (empID, empName, latedays,month, year, prevdays) VALUES ('${
        data.empID
      }', '${data.empName}', ${data.latedays}, '${data[
        "month"
      ].toLowerCase()}', ${
        data["year"]
      }, ${prevdays}) on duplicate key update latedays=${data.latedays}`,
      (err, result) => {
        if (err) {
          console.log(err);
          ...
          res.json({
            status: "success",
            message: "Late Attendance Added Successfully!",
          });
        }
      }
    );
  ...
  router.post("/miscellaneous", ensureAuthenticated, (req, res) => {
    ...
    // console.log(`INSERT INTO late_attendance (empID, empName, latedays,month, year, days) VALUES (${data.empID}, '${data.empName}', ${data.latedays}, '${data["month"]}', ${data["year"]}, ${days})`)
    mysqldb.query(
      `INSERT INTO miscellaneous (empID, empName, miscellaneous_amt ,month, year, note) VALUES ('${data.empID}', '${data.empName}', ${data.amt}, '${data["month"]}', ${data["year"]}, '${data.note}') on duplicate key update miscellaneous_amt=${data.amt},note='${data.note}'`,
      (err, result) => {
        if (err) {
          console.log(err);
          res.json({
          ...
          res.json({
            status: "success",
            message: "Miscellaneous Amount Added Successfully!",
          });
        }
      }
    );
  ...
  router.get("/storeIncomeTax", ensureAuthenticated, (req, res) => {
    mysqldb.query(
      `select * from income_tax right join Salary`,
      (err, result) => {
        if (err) {
          console.log(err);
        } else {
      ...
      if (result.length === 0) {
        ...
        mysqldb.query(`select * from config`, (err, result4) => {
          if (result.length === 0) {
            ...
            for (let i = 0; i < length; i++) {
              console.log(
                `select * from Employees left join form ORDER BY empID LIMIT ${i},1`
              );
              //to get employee specific properties for calculation
              mysqldb.query(
                `select E.empID,E.age,IFNULL(f.total,0) as total,E.emp_temp_regime,IFNULL(f.gross_sal,50000) as gross_sal from Employees E left join form f on(E.empID=f.empID) ORDER BY E.empID LIMIT ${i},1`,
                (err, result) => {
                  if (err) {
                    console.log(err);
                    console.log("error in select query of Employee");
                    ...
                    var empID = JSON.parse(JSON.stringify(result))[0].empID;
                    var age = parseInt(
                      JSON.parse(JSON.stringify(result))[0].age
                    );
                    // var investment=parseInt(JSON.parse(JSON.stringify(result))[0].investment);
                    var investment = parseInt(
                      JSON.parse(JSON.stringify(result))[0].total
                    );
                    var emp_temp_regime = JSON.parse(JSON.stringify(result))[0]
                      .emp_temp_regime;
                    var gross_total_income = parseInt(
                      JSON.parse(JSON.stringify(result))[0].gross_sal
                    );
                    ...
                    console.log("all ok");
                    console.log(
                      `insert into income_tax (empID,exemption,total_investments,tax_on_income,rebate,health_and_edu_cess,total_tax,tds_per_month,balance,month,year) VALUES ('${empID}',${exemption}${investment},${tax_on_income},0,${health_and_edu_cess},${total_tax},${tds_per_month},${balance},'${cur_month}',${cur_year})`
                    );
                    mysqldb.query(
                      `insert into income_tax (empID,exemption,total_investments,net_taxable_income,tax_on_income,rebate,health_and_edu_cess,total_tax,tds_per_month,balance,month,year,type) VALUES ('${empID}',${exemption},${investment},${net_taxable_income},${tax_on_income},${rebate},${health_and_edu_cess},${total_tax},${tds_per_month},${balance},'${cur_month}',${cur_year},"initial")`,
                      (err, result) => {
                        if (err) {
                          console.log(err);
  ...
  router.get("/register/teaching", (req, res) => {
    ...
    mysqldb.query(
      `select * from Employees right join Salary  ON Employees.empID= Salary.empID where designationCategory= "teaching" and month="${cur_month}"`,
      (err, result) => {
        if (err) {
          //console.log(err);
        } else {
  ...
  router.get("/register/nonteaching", (req, res) => {
    ...
    mysqldb.query(
      `select * from Employees right join Salary  ON Employees.empID= Salary.empID where designationCategory= 'Non-Teaching'`,
      (err, result) => {
        if (err) {
          //console.log(err);
        } else {
  ...
  router.post("/updateIncomeTax", (req, res) => {
    ...
    ihl_proof = parseInt(ihl_proof);
    e_proof = parseInt(e_proof);
    ccd_proof = parseInt(ccd_proof);
    d_proof = parseInt(d_proof);
    govt_g_proof = parseInt(govt_g_proof);
    ngo_g_proof = parseInt(ngo_g_proof);
    rent_proof = parseInt(rent_proof);

    console.log("emp is" + empID);
    console.log(req.body);
      ...
      (err, result) => {
        if (err) {
          ...
          mysqldb.query(`select * from edit_limits`, (err, upper) => {
            ...
            var climit = JSON.parse(JSON.stringify(upper))[0].climit;
            var ccdlimit = JSON.parse(JSON.stringify(upper))[0].ccdlimit;
            var ihllimit = JSON.parse(JSON.stringify(upper))[0].ihl;
            var stdlimit = JSON.parse(JSON.stringify(upper))[0].st_ded;
            var total = 0;
            console.log(total);
            ...
            if (err) {
              ...
              mysqldb.query(
                `select * from form where empID='${empID}'`,
                (err, result1) => {
                  if (err) {
                    console.log(err);
                    res.json({
                    ...
                    var mediclaim_limit = JSON.parse(JSON.stringify(result1))[0]
                      .mediclaim_limit;
                    mysqldb.query(
                      `update Employees set age=(floor(DATEDIFF(now(), dob)/ 365.2425)) where pay>0`,
                      (err, result1) => {
                          ...
                          mysqldb.query(
                            ...
                            (err, result2) => {
                              if (err) {
                                ...
                                mysqldb.query(
                                  `select ifnull(epf, 0) + ifnull(ppf, 0) + ifnull(nsc, 0) + ifnull(ulip, 0) + ifnull(insurancePremium, 0) + ifnull(houseLoan, 0) + ifnull(tuitionFee, 0) + ifnull(bankDeposits, 0) + ifnull(regFee, 0) as total from eighty_c where empID='${empID}';`,
                                  (err, result2) => {
                                    if (err) {
                                      console.log(err);
                                      res.json({
                                      ...
                                      // console.log("checking data type " + typeof(c_proof))
                                      total =
                                        (Math.min(c_proof, climit) ||
                                          Math.min(c, climit)) +
                                        (Math.min(ccd_proof, ccdlimit) ||
                                          Math.min(ccd, ccdlimit)) +
                                        (Math.min(ihl_proof, ihllimit) ||
                                          Math.min(ihl, ihllimit)) +
                                        (Math.min(d_proof, mediclaim_limit) ||
                                          Math.min(d, mediclaim_limit)) +
                                        (e_proof || e) +
                                        (govt_g_proof || donation_govt) +
                                        stdlimit;
                                      // total=0;
                                      var gg1;
                                      ...
                                      total +=
                                        Math.min(Math.min(gg1, gg2), hra) || gg;
                                      var adjusted = gross_sal - total;
                                      ...
                                      console.log("total is this " + total);
                                      mysqldb.query(
                                        `replace into proof (empID,c_proof, d_proof, ngo_g_proof, ihl_proof, govt_g_proof, e_proof, ccd_proof, rent_proof) VALUES('${empID}',${c_proof},${d_proof},${ngo_g_proof},${ihl_proof},${govt_g_proof},${e_proof},${ccd_proof},${rent_proof})`,
                                        (err, result3) => {
                                          if (err) {
                                            console.log(err);
                                            res.json({
                                            ...
                                            mysqldb.query(
                                              `select * from config left join edit_limits on config.ID = edit_limits.ID;`,
                                              (err, result4) => {
                                                if (result.length === 0) {
                                                  console.log(
                                                    "error in config query"
                                                  );
                                                } else {
                                                  var educess = JSON.parse(
                                                  ...
                                                  mysqldb.query(
                                                    ...
                                                    (err, result) => {
                                                      if (err) {
                                                        console.log(err);
                                                        console.log(
                                                          "error in select query of Employee"
                                                        );
                                                      } else {
                                                        console.log(
                                                          "Result is",
                                                        ...
                                                        mysqldb.query(
                                                          ...
                                                          (err, result) => {
                                                            ...
                                                            //take directly from salary slip
                                                              ...
                                                              console.log(
                                                                `insert into income_tax (empID,exemption,total_investments,tax_on_income,rebate,health_and_edu_cess,total_tax,tds_per_month,balance,month,year) VALUES ('${empID}',${exemption},${investment},${tax_on_income},0,${health_and_edu_cess},${total_tax},${tds_per_month},${balance_new},'${cur_month}',${cur_year})`
                                                              );
                                                              mysqldb.query(
                                                                `replace into income_tax (empID,exemption,total_investments,net_taxable_income,tax_on_income,rebate,health_and_edu_cess,total_tax,tds_per_month,balance,month,year,type) VALUES ('${empID}',${exemption},${investment},${net_taxable_income},${tax_on_income},${rebate},${health_and_edu_cess},${total_tax},${tds_per_month},${balance_new},'${cur_month}',${cur_year},"update")`,
                                                                (
                                                                  err,
                                                                  result
                                                                ) => {
  ...
  router.post("/recovery", (req, res) => {
    ...
    mysqldb.query(
      `INSERT INTO recovery (empID, month, year, recoveryAmount, note) VALUES ('${data.empID}', '${data["month"]}', ${data["year"]}, ${data.recoveryAmount}, '${data.note}') on duplicate key update recoveryAmount=${data.recoveryAmount},note='${data.note}'`,
      (err, result) => {
        if (err) {
          console.log(err);
          ...
          res.json({
            status: "success",
            message: "Recovery Amount Added Successfully!",
          });
        }
      }
    );
  ...
  router.post("/truncate", (req, res) => {
    ...
    mysqldb.query(
      `delete from ${table} where month='${cur_month}' and year=${cur_year}`,
      (err, result) => {
        if (err) {
          console.log(err);
